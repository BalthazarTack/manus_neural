...
__host__ __device__ inline bool forward_mlp_siren(const float3 &g_xyz_vs,
                                                  ...
                                                  const float g_bias2,
                                                  const float8x3 &g_weight2_color,   // NEW
                                                  const float3 &g_bias2_color,
                                                  float &integral,
                                                  float3 &color_integral)
{
    integral = 0.0f;
    ...
    float3 o_tilde = inv_rot_view * g_cano_xyz_vs * (-g_inv_max_radius);
    ...
    float3 raw_integral_color = g_bias2_color * (t1 - t0);

#pragma unroll
    for (int i = 0; i < 8; i++)
    {
        float basis = (sinf(b1_tilde.data[i] + w1_tilde.data[i] * t1) -
                       sinf(b1_tilde.data[i] + w1_tilde.data[i] * t0)) / (w1_tilde.data[i] + EPS);

        // raw_integral += w2.data[i] / (w1_tilde.data[i] + EPS) *
        //                 (sinf(b1_tilde.data[i] + w1_tilde.data[i] * t1) -
        //                  sinf(b1_tilde.data[i] + w1_tilde.data[i] * t0));
        
        raw_integral       += w2.data[i] * basis;
        raw_integral_color += g_weight2_color.rows[i] * basis;
    }
    ...
    color_integral = raw_integral_color * length_d;
...
__host__ __device__ inline bool backward_mlp_siren(
    ...
    const float8 &g_weight2,
    const float g_bias2,
    const float8x3 &g_weight2_color, 
    const float3 &g_bias2_color,
    const float &integral,
    const float3 &color_integral,  
    const float &dL_dintegral,
    const float3 &dL_dcolor_integral, 
    float3 &dL_dg_xyz_vs,
    ...
    float &dL_dg_bias2,
    float8x3 &dL_dg_weight2_color,      
    float3 &dL_dg_bias2_color)
{
    // Initialize gradients to zero
    dL_dg_xyz_vs = make_float3(0.0f, 0.0f, 0.0f);
    dL_dg_cano_xyz_vs = make_float3(0.0f, 0.0f, 0.0f);
    ...
    dL_dg_bias2 = 0.0f;
    dL_dg_weight2_color = make_float8x3(0.0f); 
    dL_dg_bias2_color = make_float3(0.0f, 0.0f, 0.0f);
    ...
    float3 raw_integral_color = color_integral / length_d;

    float dL_draw_integral = dL_dintegral * length_d;
    float3 dL_draw_integral_color = dL_dcolor_integral * length_d;

    float dL_dlength_d = dL_dintegral * raw_integral + dot(dL_dcolor_integral, raw_integral_color); // NEW: extra term

    // Compute gradients for weight2 and bias2
    dL_dg_bias2 = dL_draw_integral * (t1 - t0);
    dL_dg_bias2_color = dL_draw_integral_color * (t1 - t0); 
...
#pragma unroll
    for (int i = 0; i < 8; i++)
    {
        ...
        float basis = (sin_phi1 - sin_phi0) / term;

        // Gradient for weight2
        dL_dg_weight2.data[i]       = dL_draw_integral * basis;
        dL_dg_weight2_color.rows[i] = dL_draw_integral_color * basis; 
        
        float dL_dbasis_i = dL_draw_integral * g_weight2.data[i]
                           + dot(dL_draw_integral_color, g_weight2_color.rows[i]);

        // Gradients for w1_tilde and b1_tilde
        float common = dL_dbasis_i / term;
    }

    // Compute gradients for t0 and t1 (inclut la contribution de la couleur)
    float dL_dt0 = -dL_draw_integral * g_bias2 - dot(dL_draw_integral_color, g_bias2_color);
    float dL_dt1 = dL_draw_integral * g_bias2 + dot(dL_draw_integral_color, g_bias2_color);
...
#pragma unroll
    for (int i = 0; i < 8; i++)
    {
        ...
        dL_dt0 -= dL_dbasis_i * cos_phi0 * w1_tilde.data[i];
        dL_dt1 += dL_dbasis_i * cos_phi1 * w1_tilde.data[i];
    }
    ...
    dL_dg_inv_max_radius += dot(dL_do_tilde, inv_rot_view * g_cano_xyz_vs * (-1.0));

    // Compute gradients for g_cano_xyz_vs (from o_tilde)
    dL_dg_cano_xyz_vs += transpose(inv_rot_view) * (dL_do_tilde * (-g_inv_max_radius));
