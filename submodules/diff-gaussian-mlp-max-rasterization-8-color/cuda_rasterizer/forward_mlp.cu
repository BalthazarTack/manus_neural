...
__global__ void preprocessCUDA(int P, int D, int M,
							   ...
							   float8 *out_weight2,
							   float *out_bias2,
							   float8x3 *out_weight2_color,  
                               float3 *out_bias2_color,
							   float *rgb,
							   ...
							   bool antialiasing)
{
	...
	// Compute canonical view-space position for MLP
	if (cano_orig_points != nullptr)
	{
		float3 p_orig_cano = {cano_orig_points[3 * idx], cano_orig_points[3 * idx + 1], cano_orig_points[3 * idx + 2]};
		cano_points_xyz_view[idx] = transformPoint4x3(p_orig_cano, viewmatrix);
	}
	...
	float8x3 g_weight2_color;
	float3 g_bias2_color;
	...
	for (int i = 0; i < 8; i++)
	{
		...
		g_weight2_color.rows[i] = make_float3(
			weight2_color[idx * 24 + i * 3 + 0],
			weight2_color[idx * 24 + i * 3 + 1],
			weight2_color[idx * 24 + i * 3 + 2]);
	}
	g_bias2 = bias2[idx];
	g_bias2_color = make_float3(
		bias2_color[idx * 3 + 0],
		bias2_color[idx * 3 + 1],
		bias2_color[idx * 3 + 2]);
	...
	// Copie directe des structures (out_weight2_color est un tableau de float8x3)
	out_weight2_color[idx] = g_weight2_color;  // ✅ Accès via []
	out_bias2_color[idx] = g_bias2_color;      // ✅ Accès via []
}
...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y)
	renderMLPCUDA(
		...
		const float8x3 *__restrict__ weight2_color,
		...
		float *__restrict__ invdepth)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float integral = 0;
			float3 color_integral = make_float3(0, 0, 0);
			...
			C[0] += color_integral.x * alpha * T;
			C[1] += color_integral.y * alpha * T;
			C[2] += color_integral.z * alpha * T;
...
void FORWARD_MLP::render(
	...
	const float3 *cano_points_xyz_view,
	const float *colors,
	const float8x3 *weight1,
	const float8 *bias1,
	const float8 *weight2,
	const float *bias2,
	const float8x3 *weight2_color,
