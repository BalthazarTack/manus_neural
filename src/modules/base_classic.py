...
from src.utils.gaussian_utils_classic import density_update, render_gaussians
...
class BaseTrainingModule(LightningModule):
    ...
    def render_skeleton(self, bones_posed, camera, bg_color):
        ...
        segs = torch.cat([heads, tails], dim=0)

        N = segs.shape[0]

        colors = torch.zeros_like(segs)
        colors[:, 0] = 1.0  # green

        cov = torch.eye(3, device=segs.device).unsqueeze(0).repeat(N, 1, 1)
        scale = torch.ones(N, 3, device=segs.device) * 0.008
        rotation = torch.eye(3, device=segs.device).unsqueeze(0).repeat(N, 1, 1)

        zeros = torch.zeros(N, device=segs.device)

        rendered = render_gaussians(
            segs,
            cov,
            segs,
            torch.zeros_like(segs),
            torch.ones(N, 1, device=segs.device),
            camera,
            bg_color,
            colors_precomp=colors,
            sh_degree=0,
            tf=torch.eye(4, device=segs.device).unsqueeze(0).repeat(N, 1, 1),
        )["render"]
    ...
    def on_validation_epoch_end(self):
        ...
        row.append(self.model.get_xyz.shape[0])
