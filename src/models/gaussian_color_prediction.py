...
from src.utils.gaussian_utils import update_mask_based_on_outliers, build_symmetric
from src.utils.extra import *
import matplotlib.pyplot as plt


class GaussianModelMLP_HB(torch.nn.Module):

    def setup_functions(self):
        def build_covariance_from_scaling_rotation(scaling, scaling_modifier, rotation):
            # Normalize rotation quaternions in a numerically stable way.
    ...
    def __init__(self, opts, points=None, points_colors=None):
        super().__init__()

        if points is None:
            points = np.random.randn(opts.num_gaussians, 3)
        if points_colors is None:
            points_colors = np.random.randn(opts.num_gaussians, 3)

        n_neurons=opts.n_neurons
        self.opts = opts
        self.active_sh_degree = 0
        self.max_sh_degree = self.opts.sh_degree
        ...
        self._frequencies_color = torch.empty(0)  # [points, n_neurons, 3]
        self._phases_color = torch.empty(0)  
        self._amplitudes_color = torch.empty(0)  # [points, n_neurons, 3]
        self._offsets_color = torch.empty(0)  
        ...
        self.create_from_pcd(points, points_colors, spatial_lr_scale=1.0)
    ...
    def capture(self):
        return (
            ...
            self._amplitudes,
            self._offsets,
            self._frequencies_color,
            self._phases_color,
            self._amplitudes_color,
            self._offsets_color,    
            self.max_radii2D,
    ...
    @property
    def get_frequencies_color(self):
        return self._frequencies_color
    
    @property
    def get_phases_color(self):
        return self._phases_color
    
    @property
    def get_amplitudes_color(self):
        return self._amplitudes_color

    @property
    def get_offsets_color(self):
        return self._offsets_color
    ...
    def create_from_pcd(
        self, points, points_colors, spatial_lr_scale: float, toy_example: bool = False
    ):
        # self.spatial_lr_scale = spatial_lr_scale
        self.spatial_lr_scale = 1

        fused_point_cloud = torch.tensor(np.asarray(points)).float().cuda()
        fused_color = RGB2SH(torch.tensor(np.asarray(points_colors)).float().cuda())
        ...
        phases_color = (
            torch.rand((fused_color.shape[0], self.n_neurons)).float().cuda() * 2 * std_freq - std_freq
        )
        ...
        offsets_color = (
            torch.rand((fused_color.shape[0], 3)).float().cuda() * 2 * std_ampl - std_ampl
        )
        ...
        dist2 = torch.clamp_min(
            distCUDA2(torch.from_numpy(np.asarray(points)).float().cuda()),
            0.0000001,
        )
        ...
        self._frequencies_color = nn.Parameter(frequencies_color.transpose(1, 2).contiguous().requires_grad_(True))
        self._phases_color = nn.Parameter(phases_color.contiguous().requires_grad_(True))
        self._amplitudes_color = nn.Parameter(amplitudes_color.transpose(1, 2).contiguous().requires_grad_(True))
        self._offsets_color = nn.Parameter(offsets_color.contiguous().requires_grad_(True))
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {
                "params": [self._frequencies_color], 
                "lr": training_args.frequencies_mlp_lr, 
                "name": "frequencies_color"
            },
            {
                "params": [self._phases_color], 
                "lr": training_args.phases_mlp_lr, 
                "name": "phases_color"
            },
            {
                "params": [self._amplitudes_color], 
                "lr": training_args.amplitudes_mlp_lr, 
                "name": "amplitudes_color"
            },
            {
                "params": [self._offsets_color], 
                "lr": training_args.amplitudes_mlp_lr, 
                "name": "offsets_color"
            },
        ]
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(self._frequencies_color.shape[1] * self._frequencies_color.shape[2]):
            l.append("frequencies_color_{}".format(i))
        for i in range(self._phases_color.shape[1]):
            l.append("phases_color_{}".format(i))
        for i in range(self._amplitudes_color.shape[1] * self._amplitudes_color.shape[2]):
            l.append("amplitudes_color_{}".format(i))
        for i in range(self._offsets_color.shape[1]):
            l.append("offsets_color_{}".format(i))
    ...
    def save_ply(self, path):
        ...
        frequencies_color = (
            self._frequencies_color.detach()
            .transpose(1, 2)
            .flatten(start_dim=1)
            .contiguous()
            .cpu()
        ...
        phases_color = self._phases_color.detach().cpu().numpy() 
        amplitudes_color = (
            self._amplitudes_color.detach()
            .transpose(1, 2)
            .flatten(start_dim=1)
            .contiguous()
            .cpu()
        ...
        offsets_color = self._offsets_color.detach().cpu().numpy()    
        # ---
    ...
    def load_ply(
        ...
        if skip_mlp:
            ...
            frequencies_color =  (torch.rand((xyz.shape[0], 3, self.n_neurons)).float().cuda()
                * 2
                * std_freq
                - std_freq
            )
            phases_color = (
                torch.rand((xyz.shape[0], 3)).float().cuda() * 2 * std_freq - std_freq
            )
            amplitudes_color =  (torch.rand((xyz.shape[0], 3, self.n_neurons)).float().cuda()
                * 2
                * std_ampl
                - std_ampl
            )
            offsets_color = (
                torch.rand((xyz.shape[0], 3)).float().cuda() * 2 * std_ampl - std_ampl
            )
        else:
            # Load mlp weights
            frequencies_names = [
        ...
        self._frequencies_color = nn.Parameter(
            torch.tensor(
                frequencies_color,
                dtype=torch.float,
                device="cuda",
        ...
        self._phases_color = nn.Parameter(
            torch.tensor(
                phases_color,
                dtype=torch.float,
                device="cuda",
        ...
        self._amplitudes_color = nn.Parameter(
            torch.tensor(
                amplitudes_color,
                dtype=torch.float,
                device="cuda",
        ...
        self._offsets_color = nn.Parameter(
            torch.tensor(
                offsets_color,
                dtype=torch.float,
                device="cuda",
        ...
        with torch.no_grad():
            if removing_percent > 0 and removing_percent < 1:
                ...
                self._frequencies_color.data = self._frequencies_color.data[indices]
                self._phases_color.data = self._phases_color.data[indices]
                self._amplitudes_color.data = self._amplitudes_color.data[indices]
                self._offsets_color.data = self._offsets_color.data[indices]
    ...
    def replace_tensor_to_optimizer(self, tensor, name):
        ...
        for group in self.optimizer.param_groups:
            if group["name"] == name:
                ...
                # Get and prepare state for the new parameter
                ...
                # Remove old param from optimizer state if present
    ...
    def prune_points(self, mask):
        ...
        self._frequencies_color = optimizable_tensors["frequencies_color"]
        self._phases_color = optimizable_tensors["phases_color"]
        self._amplitudes_color = optimizable_tensors["amplitudes_color"]
        self._offsets_color = optimizable_tensors["offsets_color"]
    ...
    def remove_nan_points(self):
        """Remove points that have non-finite values in any trainable tensor."""
        nan_mask = torch.zeros(self._xyz.shape[0], dtype=torch.bool, device="cuda")

        for _, param in self.named_parameters():
            if param.shape[0] != nan_mask.shape[0]:
                continue
            param_flat = param.view(param.shape[0], -1)
            param_nan_mask = ~torch.isfinite(param_flat).all(dim=1)
            nan_mask = torch.logical_or(nan_mask, param_nan_mask)

        if nan_mask.any():
            removed = int(nan_mask.sum().item())
            self.prune_points(nan_mask)
            print(f"Removed {removed} points with NaN/Inf values")
            return removed
    ...
    def densification_postfix(
        ...
        new_amplitudes,
        new_offsets,
        new_frequencies_color,
        new_phases_color,
        new_amplitudes_color,
        new_offsets_color,
    ):
        d = {
            "xyz": new_xyz,
            ...
            "offsets": new_offsets,
            "frequencies_color": new_frequencies_color,
            "phases_color": new_phases_color,
            "amplitudes_color": new_amplitudes_color,
            "offsets_color": new_offsets_color,
        }
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, info=None):
        ...
        new_frequencies_color = self._frequencies_color[selected_pts_mask].repeat(N, 1, 1)
        new_phases_color = self._phases_color[selected_pts_mask].repeat(N,1)
        new_amplitudes_color = self._amplitudes_color[selected_pts_mask].repeat(N, 1, 1)
        new_offsets_color = self._offsets_color[selected_pts_mask].repeat(N, 1)
        # ---
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, info=None):
        ...
        new_frequencies_color = self._frequencies_color[selected_pts_mask]
        new_phases_color = self._phases_color[selected_pts_mask]
        new_amplitudes_color = self._amplitudes_color[selected_pts_mask]
        new_offsets_color = self._offsets_color[selected_pts_mask]
        # ---
    ...
    def densify_and_prune(
        self, max_grad_clone, max_grad_split, min_grad_prune, extent, max_screen_size, remove_outliers=False
    ):
        ...
        if remove_outliers:
            ...
            prune_mask = torch.logical_or(prune_mask, torch.tensor(outliers_mask, device="cuda"))
            print(f"Removing Number of outliers : {outliers_mask.sum()}")
        # if max_screen_size:
